<html>
<head>
<title>Módulo insertar Tasas de Interes</title>
</head>
<body>
<?
    // Pinto la tabla del titulo
    echo "<table align=center border=2 bgcolor='#D5E7FB' width=100%>";
    echo "<tr bgcolor='#A4B4C1'><td colspan=5 align=center><strong><font size=5>Módulo: Tasas de Interes</font></strong></td><tr bgcolor='#D5E7FB'>";
  ...
  // submitted value with another value
   function valueCompare($controlValue, $valueToCompareTo)
    ...
    $form->datePickerPath = "../../datepicker/class.datepicker.php";
        ...
        $form->add("label", "label_fechatasa", "fechatasa", "Fecha de la Gaceta:");
        $obj = & $form->add("date", "fechatasa");
        ...
        $obj->setRule(array("mandatory"=>array("e1", "Por favor! seleccione la fecha de la Gaceta para la tasa")));
        #$obj->setAttributes(array("size" =>10));   

        $form->add("label", "label_resolucion", "resolucion", "Nro. Gaceta");
        $obj = & $form->add("text", "resolucion");
        $obj->setRule(array("mandatory"=>array("e2", "Por favor! Introduzca el número de la Gaceta para item")));
        $obj->setAttributes(array("size" =>6));   
          
        $form->add("label", "label_activa", "activa", "Tasa Activa");
        $obj = & $form->add("text", "activa");
        $obj->setRule(array("mandatory"=>array("e3", "Por favor! Introduzca el Monto de la Tasa Activa")));
        $obj->setAttributes(array("size" =>6));   
        
        $form->add("label", "label_pasiva", "pasiva", "Tasa Pasiva");
        $obj = & $form->add("text", "pasiva");
        $obj->setRule(array("mandatory"=>array("e4", "Por favor! Introduzca el Monto de la Tasa Pasiva")));
        $obj->setAttributes(array("size" =>6));   
        
        $form->add("label", "label_promedio", "promedio", "Tasa Promedio");
        $obj = & $form->add("text", "promedio");
        $obj->setRule(array("mandatory"=>array("e5", "Por favor! Introduzca el Monto de la Tasa Promedio")));
        ...
        if ($form->validate()) 
        {
           // code if form is valid
            include('registrar_tasas.php');
        }
              // display the form with the specified template
        $form->render("tasas.xtpl");
...
//guardamos los valores de las variables en una sola
 ...
 $query_rsTablaPersonalizada = "SELECT id_tasa, DATE_FORMAT(fecha_tasa, '%d/%m/%Y') AS fecha, resolucion, activa, pasiva, promedio FROM tasas ORDER BY fecha ASC";
...
// creamos los parámetros de la URL: numero_pagina y total_resultados
 ...
 echo "<tr bgcolor='$color0'><th>Fecha</th><th>Gaceta</th><th>Activa</th><th>Pasiva</th><th>Promedio</th><th>Modificar</th><th>Eliminar</th></tr>";
 do 
  {
  //Creamos el java
   ...
   echo "<td>"."<a href=\"modificar_tasas.php?id=".$row_rsTablaPersonalizada['id_tasa']."\">Modificar</a>". "</td>";
   echo "<td>"."<a href=\"registrar_tasas.php?id=".$row_rsTablaPersonalizada['id_tasa']."\">Eliminar</a>". "</td>";
   echo "</tr>";
