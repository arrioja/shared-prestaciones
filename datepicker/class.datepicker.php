 ...
 *  -   you can set specific date ranges from where the user can choose from
 ...
 *  @version    1.0.3 (last revision: November 03, 2006)
...
class datePicker
{
    ...
    /**
     *  An indexed array of selectable ranges where the key is the date representing the start of the range (inclusive)
     *  and the value is the date representing the end of the range (inclusive)
     *
     *  An unlimited number of ranges can be specified.
     *
     *  In the example below we'll allow the user to select a date between tomorrow and the following 10 days
     *
     *  <code>
     *      $dp->selectableRange =
     *          array(mktime()=>strtotime("+10 days", mktime()));
     *  </code>
     *
     *  <b>Dates must be specified as unix timestamps!</b>
     *
     *  @since 1.0.2
     *
     *  @var array
     ...
     *
     *  <b>The date must be specified as a timestamp!</b>
     *
     *  @var string
     */
    var $preselectedDate;
    ...
    {
        return "javascript:var cw = null; cw = window.open('".$this->strippedPath."/includes/datepicker.php?preselectedDate=".$this->preselectedDate."&selectableRange=".urlencode(htmlspecialchars(serialize($this->selectableRange), ENT_COMPAT))."&month=".$startMonth."&year=".$startYear."&controlName=".$controlID."&dateFormat=".$this->dateFormat."&firstDayOfWeek=".$this->firstDayOfWeek."&clearDateButton=".$this->showClearDate."&language=".$this->language."&template=".$this->template."','datePicker','width=".$this->windowWidth.",height=".$this->windowHeight.",scrollbars=no,toolbar=no,menubar=no,location=no,alwaysraised=yes,modal=yes'); if (window.focus) { cw.focus() } return false";
    }
